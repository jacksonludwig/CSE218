...
public class BinaryTree {
    ...
    public boolean insert(Listing newListing) {
        ...
        } else // insert the node
        {
            ...
            n.lc = null; // not needed in java
            n.rc = null; // not needed in java
    ...
    private boolean findNode(String targetKey, TreeNodeWrapper parent,
        ...
        while (child.get() != null) {
            if (child.get().node.compareTo(targetKey) == 0) // node found
            {
                ...
                if (targetKey.compareTo(child.get().node.getKey()) < 0) { // go left if compareTo returns negative, else go right
